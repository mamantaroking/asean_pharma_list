...
df = pd.read_csv('Singapore_HSA_NewApproved_ALL.csv', encoding='ISO-8859-1')
...
layout = html.Div([
    ...
    dcc.Download(id='download-hsa'),
    dbc.Button("Download as .csv", color="primary", className="me-1", id='btn-1'),
])

@callback(
    Output('download-hsa', 'data'),
    Input('btn-1', 'n_clicks'),
    prevent_initial_call=True
)
def download_btn(click):
    return dcc.send_data_frame(df.to_csv, 'hsa_df.csv')